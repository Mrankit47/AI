const API_KEY = ""; // IMPORTANT: Secure this properly in production.
...
async function fetchGroqData(messages) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`HTTP error! status: ${response.status}, body: ${errorBody}`); // Log the full error
      throw new Error(
        `HTTP error! status: ${response.status}. Please check the console for details.` //More user-friendly
      );
    }
...
function appendMessage(content, isUser = false, isError = false) {
  ...
  chatDisplay.scrollTop = chatDisplay.scrollHeight; // Ensure scroll to bottom
}
